...
-   **Adaptive Walking Detection**: Uses moving average (MA) of peak magnitudes for dynamic threshold adaptation. Tracks last 15-20 samples (30-40s history) to establish baseline. **MA updates only during stationary periods** (when not walking) to track baseline noise, not walking peaks. Walking detected when `currentPeak > MA × 1.15` or floor clamp (0.02). Bootstrap period uses fixed threshold for first 5 samples. User-configurable MA window size (5-60 samples) persists via AsyncStorage.
-   **Consecutive Frame Confirmation**: Requires 3 consecutive detection frames (6 seconds) before starting step counting, and 3 consecutive stationary frames (6 seconds) before stopping. This eliminates phantom steps from noise spikes and residual motion, ensuring accurate start/stop transitions.
